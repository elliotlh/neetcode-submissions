class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        print(bottlenecked_time_to_arrival, 'for ', positions_and_speeds[0][0])
        for car_position, car_speed in positions_and_speeds[1:]:
            time_to_arrival = (target - car_position) / car_speed
            print('candidate TTA', time_to_arrival)
            ...
            if time_to_arrival <= bottlenecked_time_to_arrival:
                print('car', car_position, 'caught up to bottleneck because TTA', time_to_arrival, ' was <= than bottleneck', bottlenecked_time_to_arrival)
                continue
            else:
                print('car', car_position, 'is too far behind bottleneck because TTA', time_to_arrival, ' was > than bottleneck', bottlenecked_time_to_arrival)
                print('starting new fleet with', time_to_arrival, ' as the new bottleneck')
